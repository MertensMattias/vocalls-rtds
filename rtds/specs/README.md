...
This folder contains a 1–3 page Vocalls-flavoured spec for every handler in `rtds/pureconnect_handlers/`. Specs are written in Vocalls vocabulary (Params, `__rtParams`, `getValue`, `_rtNextStep`) — not PureConnect (`lsAttrNames`, `GetAt`, `creatorName`).

Each spec is the input contract for the [rtds-vocalls-component-gen](../../.claude/skills/rtds-vocalls-component-gen/SKILL.md) skill that generates the matching `rtds/components/<name>.js` mxGraph component.
...
3. Hand the spec to the `rtds-vocalls-component-gen` skill: it will generate or refresh `rtds/components/<name>.js` to match.
