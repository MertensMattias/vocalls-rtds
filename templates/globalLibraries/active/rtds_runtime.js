...
//     Writes a resolved param value to global scope.
//     - Plain name      ("foo")              -> global.foo = value
//     - Dot-notated name ("varObj.customerName") -> walks the path and
//       assigns the final segment. Intermediate segments must already exist
//       and be objects; missing or non-object segments produce a warning and
//       the write is skipped (we don't auto-create — that would mask typos).
...
function setGlobal(name, value) {
  ...
  var target = global[parts[0]];
  if (target === null || typeof target !== "object") {
    log_warn(
      `[RTDS] setGlobal: cannot assign "${name}" — root "${parts[0]}" is not an object`,
    );
    return;
  }
  for (var i = 1; i < parts.length - 1; i++) {
    ...
    if (next === null || typeof next !== "object") {
      log_warn(
        `[RTDS] setGlobal: cannot assign "${name}" — segment "${parts[i]}" is not an object`,
      );
      return;
    }
    target = next;
...
function resolveTokens(value) {
  ...
  return value.replace(/\$\(([^)]+)\)/g, function (match, name) {
    // Dot-notated tokens walk into objects on global scope (e.g. varObj.ani).
    if (name.indexOf(".") !== -1) {
      var parts = name.split(".");
      var cur = global[parts[0]];
      for (var i = 1; i < parts.length && cur !== null && cur !== undefined; i++) {
        cur = cur[parts[i]];
      }
...
function executeSetAttributes(op) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    // Names may be dot-notated to reach into nested objects (e.g. varObj.ani).
    if (key === "LogAttributes") {
      ...
      for (var j = 0; j < attrNames.length; j++) {
        ...
        if (attrName.indexOf(".") !== -1) {
          var segs = attrName.split(".");
          attrVal = global[segs[0]];
          for (var k = 1; k < segs.length && attrVal !== null && attrVal !== undefined; k++) {
            attrVal = attrVal[segs[k]];
          }
        } else {
          // Check session scope first, fall back to global scope.
          attrVal = context.session.variables[attrName];
...
function runStep(startOpId) {
  ...
  while (currentId) {
    ...
    log_debug(
      `[RTDS] Step ${current.Id} | Type: ${type} | Name: ${current.Name}`,
    );
    ...
    if (RTDS_EXIT_KEYS.has(type)) {
      ...
      log_debug(
        `[RTDS] GUI handoff step ${current.Id} (${type}) -> exit key: "${exitKey}"`,
      );
      return exitKey;
    }
