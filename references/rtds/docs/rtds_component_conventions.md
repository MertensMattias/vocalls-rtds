...
All component-scope functions are declared as assignments without `var`/`let`/`const`:
...
10. **Comments inside `Code`** use `//` single-line. Sparse JSDoc on functions only when the signature is non-obvious. No banner comments.
...
4. Populate master `Variables` with:
   ...
   - `__environment = environment;`
   - Operation-specific `__rt*` references to `_rt*` system globals.
5. Populate master `PropertiesDefinition` with the four canonical entries (`__configJSON`, `__environment`, `__nextStep`, `__outputVar`) plus any operation-specific ones.
6. Emit the three-node skeleton (`input` id=0, `init` id=7, `script` id=29, `output` id=6) with edges `0→7`, `7→29`, `29→6` (ids `28`, `30`, `38` to match the example).
7. Fill `init` `Code` with the canonical reset-and-log block.
8. Fill `script` `Code` with the operation logic (validate → set failure default → call work function → assign outcome in `.then`).
9. Add extra outputs / scripts for multi-way outcomes (one transient output per outcome).
10. Validate: master attribute set, ids numeric, edges bare orthogonal, no `arcSize=8`, no `startArrow`, single-line elements, no XML comments, `&#xa;` newlines, `&apos;` for single-quote strings.
