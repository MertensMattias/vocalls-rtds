...
**Active default — uniform `true` across all three twins AND their components** (final requester decision). Every twin mirrors its component: `setVariables.js` already defaulted `true`; `sendSms.js` / `sendMail.js` were flipped from `false` → `true` so the twin and component agree. No twin↔component divergence remains. A data op whose config has **no `Active` key** runs by default; only an explicit falsey `Active` (`false`/`0`/`"0"`/`"false"`/empty) skips. (An earlier iteration set all twins `false`; that was reversed — both the twins and the send components now default `true`.)
...
- **Inactive (`Active` resolves false) / validation fail:** stage `'nextStep'` (skip) or `'nextStep_Failure'`, return. No throw. (`Active` defaults `true` on all three twins, so the skip path is taken only when config sets a falsey `Active`.)
