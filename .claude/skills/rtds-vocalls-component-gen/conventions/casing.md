...
- `getParam(op, name, fallback)` in [rtds_2_runtime.js](../references/rtds_2_runtime.js) implements the case-insensitive fallback for the runtime side. `getValue` / `hasKey` in the env library do the same for components.
