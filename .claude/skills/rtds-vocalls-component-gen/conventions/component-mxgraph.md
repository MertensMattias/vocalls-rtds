...
The `30` (seconds) default appears in all three places and in the `__configJSON` literal default. See [externalTransfer.js:234](../references/examples/externalTransfer.js#L234) / [internalTransfer.js:204](../references/examples/internalTransfer.js#L204).
...
See the helper in [externalTransfer.js master `Code`](../references/examples/externalTransfer.js) (the `__appendPAssertedIdentity` JSDoc block) and its work-body call at [externalTransfer.js:85](../references/examples/externalTransfer.js#L85). `internalTransfer.js` omits this helper — it has no `outboundAni` Param — which is the expected shape: only add the helper when the operation actually carries a CLI Param.
