...
function getValue(obj, key, defaultValue) {
    ...
    for (var propertyName in obj) {
        if (obj.hasOwnProperty(propertyName) && String(propertyName).toLowerCase() === lowerKey) {
...
Logger = {
    ...
    resolvedPromise: function (value) {
        return {
            then: function (success) { return success(value); }
        };
    },
    ...
    sanitizeForLog: function (obj, maxSize) {
        ...
        try {
            ...
            if (json.length > maxSize) return json.substring(0, maxContentSize) + truncMarker;
            return json;
        } catch (e) {
    ...
    categorizeError: function (error, ctx) {
        var category = { type: 'UNKNOWN', source: 'APPLICATION', severity: 'ERROR' };
        ...
        if (error && (!status || typeof status !== 'number') && (!ctx || !ctx.errorCategory)) {
    ...
    buildEventDetail: function (params) {
        ...
        if (ctx && typeof ctx === 'object') {
            for (var key in ctx) {
                if (Object.prototype.hasOwnProperty.call(ctx, key)
                    && key !== 'endpoint' && key !== 'method' && key !== 'status' && key !== 'duration') {
                    extras[key] = ctx[key];
                }
            }
        }
        return {
            ...
            duration: typeof ctx.duration === 'number' ? Math.round(ctx.duration) : null,
    ...
    postEventToAPI: function (eventObj) {
        ...
        return jsonHttpRequest(apiUrl, { method: 'POST' }, _headers, requestBody)
            ...
            .then(
                function (result) { return { success: true, response: result }; },
                function (error) {
                    log_debug('[Logger] postEventToAPI error | ' + self.sanitizeForLog(error));
...
function initializeCallFlowContext(mode) {
    ...
    Logger.info('initializeCallFlowContext: ANI/DNIS extracted - ANI: ' + ani + ', DNIS: ' + dnis + ', debugCall: ' + debugCall);
...
function storeSessionVariables() {
    ...
    Logger.info('storeSessionVariables: stored varObj to session (ts: ' + timestamp + ')');
}
