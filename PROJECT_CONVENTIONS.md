...
| 6.6 | `case` parent `label=""`; each `expressionNode` child `label` equals its `Expression` verbatim (Designer validation rule) | [Component]   | grep        | [component-mxgraph.md](conventions/component-mxgraph.md)                  |
| 6.7 | `redirect`/transfer composite: `Timeout="{__transferTimeout}"` from `timeout` Param; `outboundAni` → P-Asserted-Identity via `__appendPAssertedIdentity` | [Component]   | judgment    | [component-mxgraph.md](conventions/component-mxgraph.md)                  |
