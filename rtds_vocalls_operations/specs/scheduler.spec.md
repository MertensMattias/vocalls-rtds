...
| Component name | `checkSchedule` (matches existing reference at `rtds_vocalls_operations/components/checkSchedule.js`) |
| Pattern        | `http_call` (multi-branch — branch by `result.body.action`)    |
| Source handler | `rtds_pureconnect_handlers/handlers/NAllo_RTDS_Scheduler.xml`  |
| Target file    | `rtds_vocalls_operations/components/checkSchedule.js`          |
