...
- `getParam(op, name, fallback)` in [rtds_2_runtime.js:280-296](../projects/rtds-runtime/globalLibraries/active/rtds_2_runtime.js#L280-L296) implements the case-insensitive fallback for the runtime side. `getValue` / `hasKey` in the env library do the same for components.
